...
CREATE INDEX idx_scan_results_domain
    ON osint.scans (domain);

CREATE INDEX idx_scan_results_status
    ON osint.scans (status);

CREATE UNIQUE INDEX uq_active_scan_per_domain
    ON osint.scans (domain) WHERE status IN ('IN_PROGRESS', 'QUEUED');